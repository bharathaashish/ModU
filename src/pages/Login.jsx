import { useState, useEffect } from 'react';
import { useNavigate, Link, useSearchParams } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { LogIn } from 'lucide-react';

export default function Login() {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');
  const [googleAvailable, setGoogleAvailable] = useState(false);
  const { login, loginWithGoogle } = useAuth();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();

  useEffect(() => {
    fetch('/api/auth/status')
      .then(r => r.json())
      .then(data => setGoogleAvailable(data.google))
      .catch(() => {});
  }, []);

  useEffect(() => {
    const googleUserParam = searchParams.get('google_user');
    if (googleUserParam) {
      try {
        const googleUser = JSON.parse(decodeURIComponent(googleUserParam));
        const result = loginWithGoogle(googleUser);
        if (result.success) navigate('/profile');
      } catch {
        setError('Google authentication failed');
      }
    }
    if (searchParams.get('error') === 'google_auth_failed') {
      setError('Google sign in failed. Please try again.');
    }
  }, [searchParams]);

  const handleSubmit = (e) => {
    e.preventDefault();
    setError('');
    login(email, password).then(result => {
      if (result.success) {
        navigate('/profile');
      } else {
        setError(result.message);
      }
    });
  };

  const handleGoogleLogin = () => {
    window.location.href = 'http://localhost:5001/api/auth/google';
  };

  return (
    <div className="app-container">
      <div className="glass-card">
        <h1 className="logo-text"><span style={{ fontWeight: 900 }}>M</span>od<span style={{ fontWeight: 900 }}>U</span></h1>
        {error && <div className="error-text">{error}</div>}
        <form onSubmit={handleSubmit} style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <input
            type="email"
            placeholder="Email"
            className="input"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            required
          />
          <input
            type="password"
            placeholder="Password"
            className="input"
            value={password}
            onChange={(e) => setPassword(e.target.value)}
            required
          />
          <button type="submit" className="button" style={{ marginTop: '4px' }}>
            <LogIn size={18} />
            Login
          </button>
        </form>

        {googleAvailable && (
          <>
            <div style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', marginTop: '20px' }}>
              <div style={{ flex: 1, height: '1px', backgroundColor: 'var(--border-color)' }} />
              <span style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>OR</span>
              <div style={{ flex: 1, height: '1px', backgroundColor: 'var(--border-color)' }} />
            </div>
            <button onClick={handleGoogleLogin} className="button google-btn" style={{ marginTop: '16px', backgroundColor: 'transparent', color: 'var(--text-color)', border: '1px solid var(--border-color)' }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
              Continue with Google
            </button>
          </>
        )}

        <div style={{ marginTop: '24px', fontSize: '14px' }}>
          New user? <Link to="/register" className="link">Sign up</Link>
        </div>
      </div>
    </div>
  );
}
