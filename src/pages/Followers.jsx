      ...
      try {
        ...
        if (res.ok) {
          ...
          const followerUsers = await Promise.all(
            (targetUser.followers || []).map(async (followerUsername) => {
              try {
                const userRes = await fetch(`/api/users/${followerUsername}`);
                if (userRes.ok) {
                  const followerUser = await userRes.json();
                  return { username: followerUsername, ...followerUser };
                }
              } catch (e) {
                console.error(`Failed to fetch ${followerUsername}:`, e);
              }
              return null;
            })
          );
          
          const followers = followerUsers.filter(u => u && u.username !== user.username);
  ...
  return (
    <div className="app-container" style={{ padding: '16px', justifyContent: 'flex-start', minHeight: 'calc(100vh - 120px)' }}>
      ...
      <div style={{ padding: '16px 0', display: 'flex', alignItems: 'center', gap: '12px', borderBottom: '1px solid var(--border-color)', marginBottom: '24px' }}>
        ...
        <h2 style={{ margin: 0, fontWeight: 600, fontSize: '18px', color: 'var(--text-color)' }}>Followers</h2>
      </div>
